...
esp_err_t http_server_OTA_update_handler(httpd_req_t *req)
{
	...
	do
	{
		...
		if (!is_req_body_started)
		{
			...
			esp_err_t err = esp_ota_begin(update_partition, OTA_SIZE_UNKNOWN, &ota_handle); // @suppress("Symbol is not resolved")
			if (err != ESP_OK)
			{
...
/**
 * DHT Sensor readings JSON handlr responds with DHT22 sensor data
 * @param req HTTP request for which the uri needs to be handled
 * @return ESP_OK
 */
